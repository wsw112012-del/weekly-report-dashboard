...
_STOPWORDS: set[str] = {
    ...
    # 관형격·부사격 조사 결합형 — 검색 의미 없는 grammatical noise
    "같은", "같이", "같다", "같음", "같으며", "같으면",
    "등의", "등에", "등을", "등은", "등이", "등과", "등에서", "등으로",
    "따른", "따라", "따르는", "따른다",
    "의한", "의해", "의하여", "인한", "인해", "인하여",
    "이런", "그런", "저런",
    "이러한", "그러한", "저러한",
    ...
    "함께", "모두", "각각", "각기",
    "다음", "아래", "위의",
    "매우", "너무", "아주", "정말", "약간", "다소", "거의",
    "항상", "자주", "가끔", "보통", "흔히",
    "이때", "이를", "이는", "이가", "이도", "이와", "이의",
    "그때", "그를", "그는", "그가", "그도", "그와", "그의",
}
...
_CACHE_VERSION = "v3"  # 프롬프트/후보 로직 변경 시 bump → 기존 캐시 무시
...
2) 본문 답변에서 우리 회신사례·판례를 인용할 때 [1][2] 형식, 법령 조문 인용은 [L1][L2] 형식을 사용하세요. 가능한 한 4종 출처(법령 조문 / 판례 / 유권해석 / 비조치의견서)를 골고루 인용하세요.
3) 사용자의 추가 질문이 있으면 함께 답하고, 없으면 쟁점 분석에만 집중.
4) 답변 본문은 반드시 아래 3개 라벨로 단락을 시작해 구분해 작성하세요.
   - "일치:" 로 시작하는 단락 — 첨부 검토와 DB 가 일치하거나 잘 정리된 부분
   - "보완:" 로 시작하는 단락 — 첨부 검토에서 보완·추가가 필요한 부분
   - "불명확:" 로 시작하는 단락 — 사실관계·계약·실무가 더 확인되어야 하는 부분
   각 라벨은 줄 첫머리에 단독으로 적고 콜론(:) 뒤에 본문을 이어가세요.
   해당 항목 내용이 없다면 "해당 없음" 으로 명기.
5) HTML 태그(<br>, <p>) 절대 사용하지 마세요. 단락 구분은 빈 줄로만 합니다.
6) 한국어, 마크다운 표·코드블록 사용 금지. 평어체. 강조가 필요하면 **굵게** 가능.
